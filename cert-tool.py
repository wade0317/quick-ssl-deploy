...
def get_crt(account_key, csr, acme_dir, log=LOGGER, CA=DEFAULT_CA, disable_check=False,
            directory_url=DEFAULT_DIRECTORY_URL, contact=None, check_port=None):
    ...
    def _do_request(url, data=None, err_msg="Error", depth=0, retry_count=0, max_retries=3):
        ...
        if depth < 100 and code == 400 and isinstance(resp_data, dict) and \
           resp_data.get('type') == "urn:ietf:params:acme:error:badNonce":
    ...
    if not match:
        raise ValueError("Could not parse account key - invalid format")
    pub_hex, pub_exp = match.groups()
    try:
        pub_exp = "{0:x}".format(int(pub_exp))
    except ValueError:
        raise ValueError("Invalid public exponent in account key")
    ...
    directory_url = CA + "/directory" if CA != DEFAULT_CA else directory_url  # backwards compatibility
    directory, _, _ = _do_request(directory_url, err_msg="Error getting directory")
    log.info("Directory found!")

    # create account, update contact details (if any), and set the global key identifier
    log.info("Registering account...")
    reg_payload = {"termsOfServiceAgreed": True} if contact is None else {
        "termsOfServiceAgreed": True, "contact": contact}
    account, code, acct_headers = _send_signed_request(directory['newAccount'], reg_payload, "Error registering")
    log.info("{0} Account ID: {1}".format(
        "Registered!" if code == 201 else "Already registered!", acct_headers['Location']))
    ...
    for auth_url in order['authorizations']:
        ...
        if not challenges:
            raise ValueError("http-01 challenge not found for domain {0}".format(domain))
        challenge = challenges[0]
        token = re.sub(r"[^A-Za-z0-9_\-]", "_", challenge['token'])
        keyauthorization = "{0}.{1}".format(token, thumbprint)
        # Validate the acme_dir path to prevent directory traversal
        if not os.path.isdir(acme_dir):
            raise ValueError("ACME directory does not exist: {0}".format(acme_dir))
        wellknown_path = os.path.join(acme_dir, token)
        # Double check the path is within acme_dir
        if not os.path.abspath(wellknown_path).startswith(os.path.abspath(acme_dir)):
        ...
        try:
            wellknown_url = "http://{0}{1}/.well-known/acme-challenge/{2}".format(
                domain, "" if check_port is None else ":{0}".format(check_port), token)
        ...
        try:
            ...
            log.warning("Could not remove challenge file: {0}".format(wellknown_path))
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        ...
        description=textwrap.dedent("""\
            This script automates the process of getting a signed TLS certificate from Let's Encrypt
            using the ACME protocol. It will need to be run on your server and have access to your
            private account key, so PLEASE READ THROUGH IT! It's only ~200 lines, so it won't take long.

            Example Usage:
            python cert-tool.py --account-key ./account.key --csr ./domain.csr \\
